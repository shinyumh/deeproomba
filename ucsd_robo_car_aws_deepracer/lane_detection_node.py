    ...
    def locate_centroid(self, data):
        ...
        for contour in contours:
            ...
            if self.min_width < w < self.max_width:
                try:
                    ...
                    y_list.append(y)
                    y_list.append(y+h)
                    cv2.circle(img, point1, 7, (0, 255, 0), -1)  # makes a circle at each corner
                    cv2.circle(img, point2, 7, (0, 255, 0), -1)
                    cv2.circle(img, point3, 7, (0, 255, 0), -1)
                    cv2.circle(img, point4, 7, (0, 255, 0), -1)
        ...
        try:
            if len(x_list) > 1:
                ...
                count = 0
                for x_pos in x_list:
                    error = float((x_pos - (self.image_width/2)) / (self.image_width/2))
                ...
                if abs(p_horizon_diff) <= self.error_threshold:
                    ...
                    mid_x, mid_y = x_list[error_x_index], y_list[error_x_index]
